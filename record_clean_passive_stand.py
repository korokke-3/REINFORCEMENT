...
# ★ 緑色のオブジェクト（コマンド速度マーカー等）を完全無効化・非表示にする ★
...
# シーン内の緑色マーカー（/Visuals/Command）を強制非表示
import omni.usd
...
print("\n" + "="*80)
print("=== CLEAN SCENE: ZERO GREEN OBJECTS / ZERO OBSTACLES ===")
print("="*80)
...
for step in range(80): # 1.6秒
    ...
    status = "★ PERFECT PASSIVE STANDING ★" if is_standing else "Fallen / Leaning"
...
out_dir = '/home/exhibition-spakona/Desktop/REINFORCEMENT/clean_passive_stand_proof'
os.makedirs(out_dir, exist_ok=True)

mp4_path = f"{out_dir}/clean_passive_stand.mp4"
gif_path = f"{out_dir}/clean_passive_stand.gif"

if frames:
    imageio.mimsave(mp4_path, frames, fps=25)
    imageio.mimsave(gif_path, frames, fps=25, loop=0)
    print(f"\nSaved clean video (zero green objects) to {mp4_path} and {gif_path}")
