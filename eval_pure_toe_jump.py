...
output_dir = '/home/exhibition-spakona/Desktop/REINFORCEMENT/pure_toe_jump_eval'
...
env = gym.wrappers.RecordVideo(
    ...
    name_prefix='pure_toe_jump_policy',
)
...
ckpt_path = "/home/exhibition-spakona/Desktop/REINFORCEMENT/go2_single_leg/logs/rsl_rl/2026-08-25_13-08-25/model_3997.pt"
...
print("\n" + "="*85)
print("=== EVALUATION OF PURE TOE SINGLE-LEG JUMP POLICY (KNEE CONTACT PROHIBITED) ===")
print("="*85)
...
for step in range(200):
    ...
    rr_calf = body_pos[robot.data.body_names.index('RR_calf')] # 膝
    ...
    if rr_calf[2] < min_knee_height:
        min_knee_height = rr_calf[2]
            
    if root_vel[2] > max_body_vz:
        max_body_vz = root_vel[2]
    
    is_done = dones[0].item()
    status = "★★★ PURE TOE JUMP IN AIR ★★★" if is_airborne else "Toe Ground Push"
    if is_done:
        status = "RESET"
        
    print(f"Step {step:03d} (t={step*0.02:.2f}s) | Body_Z={root_pos[2]*100:5.1f}cm | Vz={root_vel[2]:+5.2f}m/s | Foot_Z={rr_foot[2]*100:+5.1f}cm | Knee_Z={rr_calf[2]*100:+5.1f}cm | [{status}]")
...
print("\n" + "="*85)
print(f"=== PURE TOE SINGLE-LEG JUMP EVALUATION RESULTS ===")
print(f"Total Steps Evaluated  : 200 (4.00 s)")
print(f"Max Continuous Survival: {max_survival} steps ({max_survival*0.02:.2f} s)")
print(f"Max Foot Flight Height : {max_foot_height*100:.1f} cm (Ground Clearance)")
print(f"Min Knee Clearance     : {min_knee_height*100:.1f} cm (Knee Floor Clearance)")
print(f"Max Vertical Launch Vz : {max_body_vz:+.2f} m/s")
print(f"Total Airborne Steps   : {total_air_steps} steps ({total_air_steps*0.02:.2f} s in air / {total_air_steps/200*100:.1f}%)")
print("="*85)
...
# GIF生成
video_path = f"{output_dir}/pure_toe_jump_policy-step-0.mp4"
if os.path.exists(video_path):
    ...
    cap.release()
    if frames:
        gif_path = f"{output_dir}/pure_toe_jump.gif"
