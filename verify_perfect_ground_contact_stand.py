...
# ★ 足先ゴム球 (半径2.0cm) が最初から床 Z=2.0cm に 0mm 誤差でピッタリ触れている完全接地高度 ★
...
print("\n" + "="*100)
print("=== ZERO-DROP PERFECT GROUND CONTACT: STILL STANDING TEST ===")
print("="*100)
...
for step in range(80): # 1.6秒
    ...
    is_standing = rr_calf[2] > 0.05 and min_other > 0.05 and rr_foot[2] < 0.04
    status = "★ PERFECT ZERO-DROP STILL STANDING ★" if is_standing else "Reset"
    
    print(f"Step {step:03d} (t={step*0.02:.2f}s) | Base_Z={root_p[2]*100:5.2f}cm (Vz={root_v[2]:+5.2f}m/s) | Foot_Z={rr_foot[2]*100:+5.2f}cm (F={f_foot:5.1f}N) | Knee_Z={rr_calf[2]*100:+5.2f}cm | [{status}]")

out_dir = '/home/exhibition-spakona/Desktop/REINFORCEMENT/zero_drop_stand_proof'
os.makedirs(out_dir, exist_ok=True)

mp4_path = f"{out_dir}/zero_drop_still_stand.mp4"
gif_path = f"{out_dir}/zero_drop_still_stand.gif"

if frames:
    imageio.mimsave(mp4_path, frames, fps=25)
    imageio.mimsave(gif_path, frames, fps=25, loop=0)
    print(f"\nSaved zero-drop still stand video to {mp4_path} and {gif_path}")
