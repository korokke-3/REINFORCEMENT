...
parser = argparse.ArgumentParser(description="Train Go2 Right-Side Bipedal Policy")
parser.add_argument("--num_envs", type=int, default=4096, help="Number of parallel environments")
parser.add_argument("--task", type=str, default="Isaac-Velocity-Rough-Unitree-Go2-RightSide-v0", help="Task name")
...
from envs.go2_right_side_env_cfg import Go2RightSideEnvCfg


def main():
    env_cfg = Go2RightSideEnvCfg()
    ...
    print(f"[INFO] Simulating {args_cli.num_envs} Go2 robots on GPU (Right-Side Bipedal)...")

    runner = OnPolicyRunner(env, agent_cfg.to_dict(), log_dir=log_dir, device=agent_cfg.device)
    runner.learn(num_learning_iterations=agent_cfg.max_iterations, init_at_random_ep_len=True)

    env.close()
    print("[INFO] Right-Side training finished successfully!")
