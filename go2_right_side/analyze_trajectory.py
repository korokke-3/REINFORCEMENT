...
def main():
    ...
    env = gym.make('Isaac-Velocity-Rough-Unitree-Go2-RightSide-Play-v0', cfg=env_cfg)
    ...
    ckpt = max(glob.glob('/home/exhibition-spakona/Desktop/REINFORCEMENT/go2_right_side/logs/rsl_rl/**/model_*.pt', recursive=True), key=os.path.getmtime)
    ...
    print('=== RIGHT-SIDE BIPEDAL ANALYSIS (300 steps, dt=0.02s => 6.0s) ===')
    ...
    axes[1, 1].plot(time_axis, joint_positions[:, 5], label='FR_thigh (Front Right - Support)', color='teal', lw=1.5)
    axes[1, 1].plot(time_axis, joint_positions[:, 7], label='RR_thigh (Rear Right - Support)', color='purple', lw=1.5)
    axes[1, 1].set_title('Joint Angles (Right Front vs Right Rear)', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plot_path = '/home/exhibition-spakona/Desktop/REINFORCEMENT/go2_right_side/logs/trajectory_analysis.png'
    plt.savefig(plot_path, dpi=150)
    print(f"Saved right-side trajectory plots to {plot_path}")
