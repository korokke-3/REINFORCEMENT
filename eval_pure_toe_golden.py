...
ckpt_path = "/home/exhibition-spakona/Desktop/REINFORCEMENT/go2_single_leg/logs/rsl_rl/2026-08-26_22-56-41/model_2499.pt"
...
print("\n" + "="*85)
print("=== EVALUATION OF PURE TOE GOLDEN MODEL (ZERO-DROP INITIAL POSTURE) ===")
print("="*85)
...
for step in range(250): # 5.0秒間
    ...
    status = "★★★ PURE TOE AIR ★★★" if is_airborne else "Toe Rebound"
    if is_done:
        status = "RESET"
        
    print(f"Step {step:03d} (t={step*0.02:.2f}s) | Base_Z={root_pos[2]*100:5.1f}cm (Vz={root_vel[2]:+5.2f}m/s) | Foot_Z={rr_foot[2]*100:+5.1f}cm | Knee_Z={rr_calf[2]*100:+5.1f}cm | Other_Min_Z={min_other*100:+5.1f}cm | [{status}]")
...
print("\n" + "="*85)
print(f"=== PURE TOE GOLDEN MODEL EVALUATION RESULTS ===")
...
out_dir = '/home/exhibition-spakona/Desktop/REINFORCEMENT/pure_toe_golden_eval'
os.makedirs(out_dir, exist_ok=True)

mp4_path = f"{out_dir}/pure_toe_golden_jump.mp4"
gif_path = f"{out_dir}/pure_toe_golden_jump.gif"

if frames:
    imageio.mimsave(mp4_path, frames, fps=25)
    imageio.mimsave(gif_path, frames, fps=25, loop=0)
    print(f"\nSaved pure toe golden jump video to {mp4_path} and {gif_path}")
